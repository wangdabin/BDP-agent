...
if [ -d "${BIGDATA_HOME}/webapp" ];then
   BIGDATA_CLASSPATH=${BIGDATA_CLASSPATH}:${BIGDATA_HOME}/webapp
fi
if [ -d "${BIGDATA_HOME}/lib/native" ];then
   LIBRARY_PATH="-Djava.library.path=${BIGDATA_HOME}/lib/native"
fi

COMMAND="com.sky.http.SkyServer"
$JAVA_HOME/bin/java -DBIGDATA.HOME=${BIGDATA_HOME} -DSTOP.KEY=$STOP_KEY -DSTOP.PORT=$STOP_PORT -Djava.home=$JAVA_HOME $LIBRARY_PATH -cp $BIGDATA_CLASSPATH $COMMAND  > $BIGDATA_HOME/logs/skyServer.log 2>&1 &